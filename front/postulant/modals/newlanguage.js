 ...
 * @memberof Postulant
 * @ngdoc service
 * @name PostulantNewLanguageModal
 */
...
const factory = function ($modal) {
    ...
    const modalCtl = [
        ...
        "language",
        "options",
    function ($scope, PostulantService, language, options) {

        const NON_FIELD_ERRORS = 'non_field_errors';

        $scope.blockLang = false;
        ...
        $scope.$watchCollection("languageData", () => {
            $scope.apiErrors.languageForm = {};    
        });
        ...
        $scope.save = function (form) {
            ...
            data = angular.copy($scope.languageData);

            if (options.nosave) {
                return $scope.$close(data);
            }

            PostulantService.getInstance().saveLanguage(data)
                .then((data) => {
                    $scope.$close(data);
                })
                .catch((err) => {
        ...
        if (language) {
            $scope.blockLang = true;
            $scope.languageData = language;
            angular.forEach($scope.languageData, (v, k) => {
                if (typeof v === "number") {
                    $scope.languageData[k] = v.toString();
                } 
            });
        }
    ...
    const open = function (data = null, options = {}) {
        ...
        var modal = $modal.open({
            templateUrl: '/api/v1/postulant/templates/add_language.html',
            controller: modalCtl,
            backdrop: 'static',
            keyboard: false,
